...
class Supermarket extends React.Component {
  ...
  _onClickSearchSupermarket = async () => {
    if (this.state.code === '') {
      window.confirm("Debe ingresar un código de aerolinea para modificar.")
    } else {
      await fetch(`http://localhost:3000/admin/findSupermarketsId/${this.state.code}`,
        {
          method: "GET"
        })
  ...
  _submitData = async () => {
    if (this.state.name === '') {
      ...
      await fetch(`http://localhost:3000/admin/updateSupermarkets/${this.state.code}`,
        {
          method: "PUT",
            ...
            <Form>
              <Form.Row style={{ marginTop: '2%' }}>
                ...
                <Form.Group as={Col} controlId='formGridButton'>
                  <Button onClick={this._onClickSearchSupermarket}>Buscar</Button>
                </Form.Group>
              </Form.Row>
            </Form>
            <Form>
              <Form.Row style={{ marginTop: '2%' }}>
                <Form.Group as={Col} controlId='formGridCode'>
                  <Form.Label>Código</Form.Label>
                  <Form.Control type='string' placeholder={this.state.supermarket._id} disabled />
                </Form.Group>
              </Form.Row>
              ...
              <Form.Row style={{ marginTop: '2%' }}>
                <Form.Group as={Col} controlId='formGridName'>
                  ...
                  <Form.Control type='string' placeholder={this.state.supermarket.name} value={this.state.name} onChange={this._handleChangeName.bind(this)} />
                </Form.Group>
              </Form.Row>
              ...
              <Form.Row style={{ marginTop: '2%' }}>
                <Form.Group as={Col} controlId="ControlSelectAirport">
                  ...
                  <Form.Control type='string' placeholder={this.state.supermarket.airport_id} disabled />
                </Form.Group>
              </Form.Row>
              <Form.Row style={{ marginTop: '2%' }}>

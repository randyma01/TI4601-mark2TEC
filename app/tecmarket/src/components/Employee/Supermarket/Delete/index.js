...
class Supermarket extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    /*await fetch('http://localhost:3000/admin/findAllSupermarkets/',
      {
        method: "GET"
      })
  ...
  _onClickDeleteSupermarket(id) {
    ...
    console.log(verify)
    if (verify) {
      /*fetch(`http://localhost:3000/admin/deleteSupermarket/${id}`,
        {
          method: "DELETE",
          headers: {
            ...
            <tbody>
              {this.state.listSupermarkets.map((item, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <IconButton aria-label="delete" style={{ color: 'red' }} onClick={this._onClickDeleteSupermarket.bind(this, item._id)}>
                      <DeleteIcon />
                    </IconButton>
                  </td>
                </tr>
